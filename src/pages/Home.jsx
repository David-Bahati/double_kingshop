      ...
      <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b border-slate-100 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <a href="#products" className="hidden md:flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-blue-600 transition-colors bg-slate-50 px-3 py-2 rounded-xl">
              <LayoutGrid size={18} />
              Nos produits
            </a>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <button 
              ...
              className="relative p-2 bg-slate-50 rounded-full text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-all"
            >
            ...
            <Link to="/login" className="hidden sm:flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all active:scale-95">
              <UserCircle size={18} />
              Administration
            </Link>
      ...
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden bg-slate-900 pt-16">
        ...
        <div className="relative z-20 max-w-4xl px-6 text-center lg:text-left lg:ml-[-20%]">
          ...
          <p className="text-lg text-slate-300 max-w-xl mb-10 font-medium leading-relaxed">
            Votre expert à Bunia pour vos accessoires informatiques (souris, claviers, câbles) et conseils techniques avec paiement Pi Network.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
            ...
            <Link to="/login" className="px-8 py-4 bg-transparent border-2 border-white/20 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-white/10 backdrop-blur-sm transition-all text-center">
              Se connecter
            </Link>
          </div>
        </div>
            ...
            {cartItems.length > 0 && (
              <div className="mt-8 pt-8 border-t border-slate-100">
                <div className="flex justify-between items-center mb-6 px-2">
                  <span className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em]">Total à payer</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Link to="/checkout" onClick={() => setIsCartOpen(false)} className="py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest text-center shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all">Valider</Link>
                </div>
              </div>
            )}
          </div>
      ...
      <main id="products" className="max-w-7xl mx-auto px-4 py-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 px-4">
          ...
          <div className="flex gap-2">
            {['Tout', 'Souris', 'Claviers', 'Câbles'].map(cat => (
              <button key={cat} className="px-4 py-2 rounded-xl text-xs font-bold border border-slate-200 hover:border-blue-600 hover:text-blue-600 transition-all bg-white">{cat}</button>
            ))}
          </div>
        </div>
